...
function easycontactforms_entrypoint() {
	...
	$client = new SoapClient('http://www.cytrack.com/Licence/LicenceService.asmx?wsdl');
	if($map["hidden-19"] == "ufo-form-id-19"){
		$response = $client->GetReLicence(array(
			'cdcode' => $map['id-3931'],
			'systemcode' => $map['id-3937']
		));
	}elseif($map['hidden-15'] == "ufo-form-id-15"){
		$response = $client->GetLicence(array(
			'cdcode' => $map['id-3722'],
			'systemcode' => $map['id-3736'],
			'emailAddress' => $map['id-3731'],
			'contactName' => $map['id-3723'],
			'companyName' => $map['id-3724'],
			'resellerName' => $map['id-3732'],
			'pabxSerialNumber' => $map['id-3725'],
			'phoneNumber' => $map['id-3726'],
			'webPage' => $map['id-3737'],
			'street' => $map['id-3727'],
			'suburb' => $map['id-3738'],
			'region_city' => $map['id-3728'],
			'state' => $map['id-3735'],
			'county' => $map['id-3750'],
			'country' => $map['id-3753'],
			'postcode' => $map['id-3740']
		));
	}
	
	if(empty($response->GetReLicenceResult->Result)){
		$message = $response->GetReLicenceResult->ErrorString;
		$errorMsg = true;
	}else{
		$errorMsg = false;
		$message = "License Passcode : ".$response->GetReLicenceResult->LicenceCode;
	}
	$map['errMsg'] = $errorMsg;
	$map['appendMsg'] = $message;
